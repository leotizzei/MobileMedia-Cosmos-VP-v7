...
import br.unicamp.ic.sed.mobilemedia.photo.spec.excep.*;
...
public aspect XPIPhotoController {
	...
	declare soft : PersistenceMechanismException : 
		deleteImage() || updateImage() || callShowImage() || addNewPhotoToAlbum();
		
	declare soft : ImageNotFoundException :
		callShowImage()	|| editLabel() || deleteImage();
	
	declare soft : InvalidImageDataException :
		addNewPhotoToAlbum() || updateImage();
	
	declare soft : NullAlbumDataReference :
		editLabel();
